/*
 * HJM's CVS versioning:
 * $Id: seqio.c,v 1.2 2005/01/24 21:23:47 mangalam Exp $
 *
...
#include <sys/types.h>
#include <sys/stat.h>
/* hjm fix to allow OSX to find dirent.h - 5.29.03 */
#if defined (__unix) || defined (__linux__) || defined (__APPLE__)
#include <unistd.h>
#include <dirent.h>
#ifdef SYSV
...
/* this next bit causes problems b/c of the way IRIX, some other OSs like
theirs defined  */
/* added hjm - 10.11.99 */
...
#define s_toupper(chr) tuary[(int) (chr)]
...
static int mycasecmp(char *s, char *t)
{
    ...
    for ( ; !(diff = s_toupper(*s) - s_toupper(*t)) && *s; s++,t++) ;
    return diff;
}
...
static int myncasecmp(char *s, char *t, int n)
{
    ...
    diff = 0;
    for (i=0; i < n && !(diff = s_toupper(*s) - s_toupper(*t)) && *s; s++,t++,i++) ;
    return diff;
}
...
#define mystreq(s1,ch,s2)  (s_toupper(*(s1)) == (ch) && mystreqfn((s1),(s2)))
static int mystreqfn(char *s1, char *s2)
{
    int diff;
    while (!(diff = s_toupper(*++s1) - *++s2) && *s2) ;
    return !*s2;
}
                            ...
                            int field)
{
    ...
    case SEQINFO_DBNAME:
        s = isfp->db_name;
        break;
    case SEQINFO_FILENAME:
        s = isfp->filename;
        break;
    case SEQINFO_FORMAT:
        switch (isfp->format) {
        case FORMAT_GBFAST:
            s = file_table[FORMAT_GENBANK].ident;
            break;
        case FORMAT_PIRFAST:
            s = file_table[FORMAT_PIR].ident;
            break;
        case FORMAT_EMBLFAST:
            s = file_table[FORMAT_EMBL].ident;
            break;
        case FORMAT_SPFAST:
            s = file_table[FORMAT_SPROT].ident;
            break;
        case FORMAT_GCG:
...
static int intseqf_field3(SEQFILE *sfp, char *fnname, int field)
{
    ...
    switch (field) {
    case SEQINFO_FRAGMENT:
        return isfp->info->isfragment;
    case SEQINFO_CIRCULAR:
        return isfp->info->iscircular;
    case SEQINFO_ALPHABET:
        return isfp->info->alphabet;
    case SEQINFO_STARTPOS:
        return isfp->info->fragstart;
    case SEQINFO_TRUELEN:
        return isfp->entry_truelen;
    case SEQINFO_RAWLEN:
        return isfp->entry_rawlen;
    case SEQINFO_ENTRYNO:
        return isfp->entry_count;
    case SEQINFO_SEQNO:
        return isfp->entry_seqno;
    case SEQINFO_NUMSEQS:
        return isfp->entry_numseqs;
    default:
        return 0;
    }
}
...
SEQINFO *seqfparseent(char *entry, int entrylen, char *format)
{
    ...
    /* memset(isfp->info, isfp->infosize, 0);  hjm - above line transposed? */
...
static int determine_format(INTSEQFILE *isfp)
{
    ...
    case STATUS_OK:
        break;
    case STATUS_EOF:
        raise_error(E_DETFAILED, return STATUS_ERROR,
    ...
    case STATUS_FATAL:
        return STATUS_FATAL;
    default:
        status_error(return STATUS_ERROR, "determine_format");
    }
    ...
    for (i=0; i < file_table_size; i++) {
        ...
        while (*det != '\0') {
            ...
            for (s=text; *s && *det && *det != '|'; s++,det++)
                if (*det != '?' && s_toupper(*s) != s_toupper(*det))
                    break;
...
static char *intseqf_info(INTSEQFILE *isfp, int newbuffer, int flag)
{
    ...
    if (flag == SEQINFO_ALL || flag == SEQINFO_ALLINFO) {
        ...
        case SEQINFO_IDLIST:
            ...
            s = isfp->info->idlist;
            break;
        case SEQINFO_DESCRIPTION:
            isfp->iflag_description = 1;
            s = isfp->info->description;
            break;
        case SEQINFO_COMMENT:
            isfp->iflag_comment = 1;
            s = isfp->info->comment;
            break;
        case SEQINFO_ORGANISM:
            isfp->iflag_organism = 1;
            s = isfp->info->organism;
            break;
        case SEQINFO_FRAGMENT:
            isfp->iflag_fragment = 1;
            return "";
        case SEQINFO_CIRCULAR:
            isfp->iflag_circular = 1;
            return "";
        case SEQINFO_ALPHABET:
            isfp->iflag_alphabet = 1;
            return "";
        case SEQINFO_STARTPOS:
            isfp->iflag_fragstart = 1;
            return "";
        case SEQINFO_TRUELEN:
        case SEQINFO_RAWLEN:
            return "";
...
static int raw_read(INTSEQFILE *isfp, int flag)
{
    ...
    case STATUS_FATAL:
        return STATUS_FATAL;
    default:
        status_error(return STATUS_ERROR, "raw_read");
    }
...
static int databank_read(INTSEQFILE *isfp, int flag)
{
    ...
    case STATUS_OK:
        break;
    case STATUS_EOF:
        error_test(isfp->format != FORMAT_GCG,
                   E_PARSEERROR, return STATUS_ERROR,
...
static int basic_read(INTSEQFILE *isfp, int flag)
{
    ...
    case STATUS_OK:
        ...
        break;
    case STATUS_EOF:
        isfp->fp_entryend = isfp->fp_current = isfp->fp_top;
        break;
    case STATUS_ERROR:
        return STATUS_ERROR;
    case STATUS_FATAL:
    ...
    if (descr_line != -1) {
        ...
        if (s - line >= 3 && isspace(*(s-2)) &&
                ((s_toupper(*(s-1)) == 'B' && s_toupper(*s) == 'P') ||
                 (s_toupper(*(s-1)) == 'A' && s_toupper(*s) == 'A') ||
                 (s_toupper(*(s-1)) == 'C' && s_toupper(*s) == 'H'))) {
            flag++;
        ...
        case FORMAT_SPFAST:
            ...
            while (status == STATUS_OK) {
                ...
                if (s < top) {
                    if (s[-12] == '\n' && s[-11] == 'S' && s[-10] == 'Q' &&
                            s[-9] == ' ' && s[-8] == ' ' && s[-7] == ' ' && s[-6] == 'S' &&
                            s_toupper(s[-5]) == 'E' && s_toupper(s[-4]) == 'Q' &&
                            s_toupper(s[-3]) == 'U' && s_toupper(s[-2]) == 'E' &&
                            s_toupper(s[-1]) == 'N')
                        break;
                    else
                        s += 13;
...
static int fastaout_read(INTSEQFILE *isfp, int flag)
{
    ...
    if (isfp->entry_count == 0) {
        ...
        case STATUS_FATAL:
            return STATUS_FATAL;
        default:
            status_error(return STATUS_ERROR, "fastaout_read");
        }
        ...
        while ((status = fp_get_line(isfp, &line, &end)) == STATUS_OK) {
            if (mystreq(line, 'Q', "QUERY=") ||
                    (mystreq(line, 'B', "BLAST") &&
                     ((ch = s_toupper(line[6])) == 'N' || ch == 'P' || ch == 'X')))
                break;
        }

        if (status == STATUS_OK && s_toupper(line[0]) == 'B') {
...
static int phylip_read(INTSEQFILE *isfp, int flag)
{
    ...
    case STATUS_FATAL:
        return STATUS_FATAL;
    default:
        status_error(return STATUS_ERROR, "phylip_read");
    }
...
static int clustal_read(INTSEQFILE *isfp, int flag)
{
    ...
    if (isfp->entry_count == 0) {
        ...
        case STATUS_FATAL:
            return STATUS_FATAL;
        default:
            status_error(return STATUS_ERROR, "clustal_read");
        }
...
static int asn_read(INTSEQFILE *isfp, int flag)
{
    ...
    char ch, *line, *end, *s, *t, *lenstr, *lenend, *seqstr;
            ...
            case '\'':
                ...
                while (1) {
                    ...
                    case STATUS_FATAL:
                        return STATUS_FATAL;
                    default:
                        status_error(return STATUS_ERROR, "asn_read");
                    }
                    s = line;
                }
                s++;
...
static int fp_read_all(INTSEQFILE *isfp)
{
    ...
    while (status == STATUS_OK) {
        ...
        case STATUS_FATAL:
            return STATUS_FATAL;
        default:
            status_error(return STATUS_ERROR, "fp_read_all");
        }
    }
    isfp->fp_current = line;
    isfp->fp_top = top;
...
static int setup_field(INFO *info, int field, int len)
{
    ...
    switch (field) {
    case SEQINFO_FILENAME:
        fieldptr = &info->info->filename;
        break;
    case SEQINFO_DBNAME:
        fieldptr = &info->info->dbname;
        break;
    case SEQINFO_FORMAT:
        fieldptr = &info->info->format;
        break;
    case SEQINFO_IDLIST:
        fieldptr = &info->info->idlist;
        break;
    case SEQINFO_DATE:
        fieldptr = &info->info->date;
        break;
    case SEQINFO_DESCRIPTION:
        fieldptr = &info->info->description;
        break;
    case SEQINFO_COMMENT:
        fieldptr = &info->info->comment;
        break;
    case SEQINFO_ORGANISM:
        fieldptr = &info->info->organism;
        break;
    case SEQINFO_HISTORY:
        fieldptr = &info->info->history;
        break;
    default:
        return 0;
    }
...
static int get_alphabet(char *s, char *end)
{
    ...
    if (end - s > 3 && s[2] == '-' && s_toupper(s[1]) == 'S' &&
            ((ch = s_toupper(s[0])) == 'D' || ch == 'S' || ch == 'M'))
        s += 3;

    if (end - s < 2)
        return UNKNOWN;

    if (end - s >= 2 && s_toupper(*(end-2)) == 'N' && s_toupper(*(end-1)) == 'A') {
        if (end - s == 2)
            return DNA;
...
static void parse_oneline(INFO *info, char *start, char *end, int info_flag)
{
    ...
    if (s - 1 >= start) {
        s--;
        if (((s_toupper(*s) == 'A' && s_toupper(s[1]) == 'A' && (i = 1)) ||
                (s_toupper(*s) == 'B' && s_toupper(s[1]) == 'P' && (i = 2)) ||
                (s_toupper(*s) == 'C' && s_toupper(s[1]) == 'H' && (i = 3)))) {
...
static int genbank_getinfo(INTSEQFILE *isfp, char *entry, int len, int flag)
{
    ...
    while ((status = gi_getline(&line, &end, 1)) &&
        ...
        case 'N':
        case 'P':
            if (s_toupper(line[1]) == 'I' && s_toupper(line[2]) == 'D' &&
...
static int nbrf_getinfo(INTSEQFILE *isfp, char *entry, int len, int flag)
{
    ...
    if ((ch == 'P' || ch == 'F') && line[2] == '1') {
        ...
        if (allflag || flag == SEQINFO_CIRCULAR) {
            set_circular(&info, s_toupper(line[2]) == 'C');
            if (flag == SEQINFO_CIRCULAR)
                goto NBRF_GI_END;
        }
    }
...
static int asn_getinfo(INTSEQFILE *isfp, char *entry, int len, int flag)
{
    ...
    if (destr != NULL && (allflag || flag == SEQINFO_DATE)) {
        ...
        if (cdate != NULL)  {
            ...
            date = udate;
            dend = udend;
        } else if (gbedate != NULL)  {
            date = gbedate;
            dend = gbeend;
        } else if (emudate != NULL)  {
            date = emudate;
            dend = emuend;
        } else if (emcdate != NULL)  {
            date = emcdate;
            dend = emcend;
        } else if (spadate != NULL)  {
            date = spadate;
            dend = spaend;
        } else if (spudate != NULL)  {
            date = spudate;
            dend = spuend;
        } else if (spcdate != NULL)  {
            date = spcdate;
            dend = spcend;
        } else if (pdbdate != NULL)  {
            date = pdbdate;
            dend = pdbend;
        } else {
            exists = 0;
...
{
    int i, j, k, alpha, align_flag, count;
                        ...
                        int flag)
{
    ...
    history = NULL;
    count = hcount = 0;
    if (s_toupper(*line) == 'C' && s_toupper(line[1]) == 'O') {
                         ...
                         int flag)
{
    ...
    ccflag = (s_toupper(*line) == 'C');
    xxflag = (s_toupper(*line) == 'X');
                        ...
                        int flag)
{
    ...
    char *destr, *deend, *str, *strend, *lastcomment;
        ...
        for ( ; s < fileend && t < patend; s++,t++) {
            ...
            if (count == 0) {
                ...
                for (s2--,t2--; s2 > s && t2 > t; s2--,t2--)
                    if (!(s_toupper(*s2) == s_toupper(*t2) || *s2 == '?' || *t2 == '?'))
                        return 0;
    ...
    if (*s == '*') {
        ...
        for (temp=t; temp < tend; temp++) {
            for (s2=s+1,t2=temp; s2 < send && t2 < tend; s2++,t2++) {
                if ((s_toupper(*s2) == s_toupper(*t2) && *s2 != '*') ||
                        *s2 == '?' || *t2 == '?')
                    continue;
...
static int findline(FILE *fp, int low, int high, char *pattern, int patsize)
{
    int middle, pagesize, status, offset, highflag, len;
